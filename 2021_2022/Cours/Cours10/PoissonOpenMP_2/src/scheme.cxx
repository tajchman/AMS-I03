                                 ...
                                 int kmin, int kmax)
{
  ...
  #pragma omp parallel for reduction(+:du_sum) shared(lam_x, lam_y, lam_z, xmin, ymin, zmin, imin, jmin, kmin, imax, jmax, kmax) private(x,y,z,i,j,k,du,du1,du2) default(none)
  for (k = kmin; k <= kmax; k++) 
    for (j = jmin; j <= jmax; j++)
      for (i = imin; i <= imax; i++)
      {
