...
public:
    ...
    inline bool            long_options_contain(const char* FlagList) const;
    ...
    inline long               operator()(const char* VarName, long          Default) const;
...
GetPot::nominus_followers(const char* Option)
{
    ...
    while( 1 + 1 == 2 ) {
        ++cursor;
        if( cursor >= argv.size() ) { 
            cursor = static_cast<unsigned int>(argv.size() - 1);
            return result_list;
        }
...
inline bool
GetPot::long_options_contain(const char* FlagList) const
{
    // go through all arguments that start with a '--'
    std::string str;
    STRING_VECTOR::const_iterator it = argv.begin();
    for(; it != argv.end(); ++it) {
        str = __get_remaining_string(*it, prefix);
	  ...
	  if( __check_flags(str2, FlagList) ) return true;
	}
    }
    return false;
}
...
inline long
GetPot::operator()(const char* VarName, long Default) const
{
    // (*) recording of requested variables happens in '__find_variable()'
    const variable*  sv = __find_variable(VarName);
    if( sv == 0 ) return Default;
...
{ __set_variable(VarName, __long2string(Value).c_str()); }
