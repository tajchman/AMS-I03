...
bool Scheme::solve(unsigned int nSteps)
{
  ...
  double du_sum, du_inner_sum, du_faces_sum;
...
#pragma omp section
      {
        ...
        for (i = imin + di; i < imax - di; i++)
          for (j = jmin + dj; j < jmax - dj; j++)
            for (k = kmin + dk; k < kmax - dk; k++) {
...
#pragma omp section
      {
        ...
        for (j = jmin; j < jmax; j++)
          for (k = kmin; k < kmax; k++) {
            if (j < jmin || j >= jmax || k < kmin || k >= kmax) {
              std::cout << "(_, j, k) = (" << "_" << ", " << j << ", " << k << ")" << std::endl;
            }
            du = 6 * m_u(i, j, k) 
                - m_u(imin + di, j, k) - m_u(imin - di, j, k)
                - m_u(imin, j + dj, k) - m_u(imin, j - dj, k) 
                - m_u(imin, j, k + dk) - m_u(imin, j, k - dk);
            du *= lambda;
            m_v(i, j, k) = m_u(i, j, k) - du;
            du_faces_sum += du > 0 ? du : -du;
          }
        ...
        for (j = jmin; j < jmax; j++)
          for (k = kmin; k < kmax; k++) {
            ...
            du = 6 * m_u(i, j, k) 
                - m_u(imax + di, j, k) - m_u(imax - di, j, k)
                - m_u(imax, j + dj, k) - m_u(imax, j - dj, k) 
                - m_u(imax, j, k + dk) - m_u(imax, j, k - dk);
            du *= lambda;
            m_v(i, j, k) = m_u(i, j, k) - du;
            du_faces_sum += du > 0 ? du : -du;
          }
        ...
        for (i = imin; i < imax; j++)
          for (k = kmin; k < kmax; k++) {
            if (i < imin || i >= imax || k < kmin || k >= kmax) {
              std::cout << "(i, _, k) = (" << i << ", " << "_" << ", " << k << ")" << std::endl;
            }
            du = 6 * m_u(i, j, k) 
                - m_u(i + di, jmin, k) - m_u(i - di, jmin, k)
                - m_u(i, jmin + dj, k) - m_u(i, jmin - dj, k) 
                - m_u(i, jmin, k + dk) - m_u(i, jmin, k - dk);
            du *= lambda;
            m_v(i, j, k) = m_u(i, j, k) - du;
            du_faces_sum += du > 0 ? du : -du;
          }
        ...
        for (i = imin; i < imax; j++)
          for (k = kmin; k < kmax; k++) {
            ...
            du = 6 * m_u(i, j, k) 
                - m_u(i + di, jmax, k) - m_u(i - di, jmax, k)
                - m_u(i, jmax + dj, k) - m_u(i, jmax - dj, k) 
                - m_u(i, jmax, k + dk) - m_u(i, jmax, k - dk);
            du *= lambda;
            m_v(i, j, k) = m_u(i, j, k) - du;
            du_faces_sum += du > 0 ? du : -du;
          }
        ...
        for (i = imin; i < imax; j++)
          for (j = jmin; j < jmax; k++) {
            if (i < imin || i >= imax || j < jmin || j >= jmax) {
              std::cout << "(i, j, _) = (" << i << ", " << j << ", " << "_" << ")" << std::endl;
            }
            du = 6 * m_u(i, j, k) 
                - m_u(i + di, j, kmin) - m_u(i - di, j, kmin)
                - m_u(i, j + dj, kmin) - m_u(i, j - dj, kmin) 
                - m_u(i, j, kmin + dk) - m_u(i, j, kmin - dk);
            du *= lambda;
            m_v(i, j, k) = m_u(i, j, k) - du;
            du_faces_sum += du > 0 ? du : -du;
          }
        ...
        for (i = imin; i < imax; j++)
          for (j = jmin; j < jmax; k++) {
            ...
            du = 6 * m_u(i, j, k) 
                - m_u(i + di, j, kmax) - m_u(i - di, j, kmax)
                - m_u(i, j + dj, kmax) - m_u(i, j - dj, kmax) 
                - m_u(i, j, kmax + dk) - m_u(i, j, kmax - dk);
            du *= lambda;
            m_v(i, j, k) = m_u(i, j, k) - du;
            du_faces_sum += du > 0 ? du : -du;
          }
    ...
    m_timers[2].start();
    du_sum = du_inner_sum + du_faces_sum;
