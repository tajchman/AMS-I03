...
{	
  ...
  int iT = omp_get_thread_num();
	imin = m_p.imin_thread(0, iT);
	imax = m_p.imax_thread(0, iT);
	jmin = m_p.imin_thread(1, iT);
	jmax = m_p.imax_thread(1, iT);
	kmin = m_p.imin_thread(2, iT);
	kmax = m_p.imax_thread(2, iT);
	#else
	imin = m_imin[0];
	imax = m_imax[0];
	jmin = m_imin[1];
