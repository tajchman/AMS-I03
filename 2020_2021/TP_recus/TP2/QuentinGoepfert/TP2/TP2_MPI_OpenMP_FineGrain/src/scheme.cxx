                                 ...
                                 int kmin, int kmax)
{
  ...
  // On parallelise les trois boubles
  # pragma omp parallel for private(x, y, z, du1, du2, du) default(shared) reduction(+: du_sum_local) collapse (3)
  for (i = imin; i <= imax; i++)
    for (j = jmin; j <= jmax; j++)
      for (k = kmin; k <= kmax; k++) {
    ...
    // Appel MPI par un seul thread: le thread maitre
