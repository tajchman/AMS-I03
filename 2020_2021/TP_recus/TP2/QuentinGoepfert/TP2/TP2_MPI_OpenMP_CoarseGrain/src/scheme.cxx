...
bool Scheme::iteration()
{
  ...
  double duv_th = iteration_domaine(
  ...
  #pragma omp single
  {
    // Creation d'une variable m_du_threads dans Scheme
    // Elle est partagee par les threads
    m_du_threads = 0.0;
    m_t += m_dt;
    m_u.swap(m_v);
  }
  ...
  // Appel MPI par le premier thread qui a fini (d'ou la consigne MPI_THREAD_MULTIPLE dans MPI_INIT_thread)
  # pragma omp single
  {
    MPI_Allreduce(&m_du_threads, &m_duv, 1, MPI_DOUBLE, MPI_SUM, m_P.comm());
  }
