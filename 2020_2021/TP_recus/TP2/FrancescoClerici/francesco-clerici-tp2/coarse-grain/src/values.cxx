...
void Values::init()
{
  ...
  int imin_th = m_p.imin_thread(0, id_th); // divide the MPI-domain in nthreads-slices 
  int imax_th = m_p.imax_thread(0, id_th);

  for (i=imin_th; i<=imax_th; i++)
...
void Values::boundaries(callback_t f)
{
  ...
  for (int idim=0; idim<3; idim++) {
    ...
    pmin_th = m_p.imin_thread(jdim, id_th); // divide the MPI-domain in nthreads-slices 
    pmax_th = m_p.imax_thread(jdim, id_th);
