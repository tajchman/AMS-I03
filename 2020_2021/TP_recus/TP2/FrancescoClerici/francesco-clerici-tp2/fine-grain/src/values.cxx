...
void Values::init()
{
  ...
  // parallel here
  # pragma omp parallel for num_threads(m_p.nthreads()) private(j,k)
...
void Values::init(callback_t f)
{
  ...
  // parallel here
  # pragma omp parallel for num_threads(m_p.nthreads()) private(j,k,p,x)
  for (i=m_imin[0]; i<=m_imax[0]; i++)
    for (j=m_imin[1]; j<=m_imax[1]; j++)
...
void Values::boundaries(callback_t f)
{
  ...
  for (int idim=0; idim<3; idim++) {
    ...
    if (m_p.neighbour(2*idim) < 0) {
      ...
      x[idim] = m_xmin[idim];
	  // parallel here
      //# pragma omp parallel for private(q,i,x)
      for (p=pmin-1; p<=pmax+1; p++)
        for (q=qmin-1; q<=qmax+1; q++) {
          ...
          operator()(i) = f(x); // segfault: unconsistent value of i[1]
        }
    }
