...
// Initiation de MPI
  MPI_Init(&argc, &argv);
// Parametres de processus courant et nombre de processus
  int rank, size;
...
// Parameter.cxx
  ...
  int itMax = Prm.itmax(); // nombre d'itération dans boucle du temps
  int freq = Prm.freq();  // freq de sortie des résultats
...
// Scheme.cxx
  ...
  MPI_Barrier(Prm.comm()); // Block de MPI
  if (Prm.rank() == 0) {
    std::cout << "\n  temps init "
  ...
  // Calcul des itérataires de temps
  ...
  for (int it=0; it < itMax; it++) {
    ...
    T_comm.start();
    C.synchronize(); // transfer les valeurs au bord vers domaine de voisin
    T_comm.stop();

    T_calcul.start();
    C.iteration(); // calcule une itération
    T_calcul.stop();
