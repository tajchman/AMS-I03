...
Scheme::Scheme(Parameters &P, callback_t f, int numThreads) :
...
bool Scheme::iteration(int iThread,int numThreads)
{
	
	#ifdef _OPENMP
  m_temp[iThread] = iteration_domaine(
	...
	{
	m_duv = 0.0;
	for (int thread=0;thread<numThreads;thread++) m_duv+=m_temp[thread];
  m_t += m_dt;
  m_u.swap(m_v);
	}
	#else
