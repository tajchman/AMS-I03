...
void Values::init()
{
  ...
  int imin_loc = m_p.imin_local(0, tid); // divide the domain in nthreads-slices of dimension (dx/3,dy,dz)
  int imax_loc;
...
void Values::init(callback_t f)
{
  ...
  int tid = omp_get_thread_num();
  int imin_loc = m_p.imin_local(0, tid); // divide the domain in nthreads-slices of dimension (dx/nthreads,dy,dz)
  int imax_loc;
  int nth = m_p.nthreads();
...
void Values::boundaries(callback_t f)
{
  ...
  int tid = omp_get_thread_num();
  int imin_loc = m_p.imin_local(0, tid); // divide the domain in nthreads-slices of dimension (dx/nthreads,dy,dz)
  int jmin_loc = m_p.imin_local(1, tid); // divide the domain in nthreads-slices of dimension (dx,dy/nthreads,dz)
  int imax_loc, jmax_loc;
  int nth = m_p.nthreads();
  ...
  if(tid == nth-1) {
    ...
    imax_loc = m_p.imin_local(0, tid+1);
	jmax_loc = m_p.imin_local(1, tid+1);
  }

  for (j=jmin_loc; j<jmax_loc; j++)
    for (k=kmin; k<kmax; k++) 
    {
