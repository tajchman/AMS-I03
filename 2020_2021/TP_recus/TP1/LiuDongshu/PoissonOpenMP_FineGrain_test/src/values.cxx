...
default(shared) private(i,j,k) collapse(3)
  for (i=imin; i<imax; i++)
    for (j=jmin; j<jmax; j++)
      for (k=kmin; k<kmax; k++)
        operator()(i,j,k) = 0.0;
...
void Values::boundaries(callback_t f)
{
  int i, j, k;
#pragma  omp  parallel  for \
default(shared) private(j,k) collapse(2)
  for (j=jmin; j<jmax; j++)
    for (k=kmin; k<kmax; k++) 
    {
...
#pragma  omp  parallel  for \
default(shared) private(i,k) collapse(2)
  for (i=imin; i<imax; i++)
    for (k=kmin; k<kmax; k++)
    {
...
#pragma  omp  parallel  for \
default(shared) private(i,j) collapse(2)
  for (i=imin; i<imax; i++)
    for (j=jmin; j<jmax; j++)
    {
