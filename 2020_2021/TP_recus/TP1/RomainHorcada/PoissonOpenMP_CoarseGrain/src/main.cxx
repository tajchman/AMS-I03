...
int main(int argc, char *argv[])
{
  ...
  {
  int i_min = Prm.imin_local(0,omp_get_thread_num());
  int i_max = Prm.imax_local(0,omp_get_thread_num());
  int j_min = Prm.imin_local(1,omp_get_thread_num());
  int j_max = Prm.imax_local(1,omp_get_thread_num());
  int k_min = Prm.imin_local(2,omp_get_thread_num());
  int k_max = Prm.imax_local(2,omp_get_thread_num());
  for (int it=0; it < itMax; it++) {
    ...
    T_calcul.start();
    #pragma omp single
    C.iteration(i_min, i_max, j_min, j_max, k_min, k_max);
    T_calcul.stop();
