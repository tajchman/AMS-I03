...
\begin{minipage}{\textwidth}
		...
		\begin{enumerate}
			...
			\item \color{blue}Ajouter les {\tt pragmas}
			\item \color{blue}Identifier les variables partagées et privées
			\item \color{blue}Compiler, lancer le code
			\item \color{blue}Comparer avec la version séquentielle
			\item \color{blue}Si différent, corriger le code parallélisé et revenir en (4)
		\end{enumerate}
	
	\bigskip
	    Envoyez votre code source modifié (compressé dans une archive), avec les résultats obtenus (copie des sorties écrans et fichiers pdf obtenus avec le script run.py) {\bf le 20/12/2020 au plus tard}.
	\end{minipage}
}
...
\begin{frame}
	...
	\framebox{\color{blue}%
		\begin{minipage}{\textwidth}
			Dans le répertoire {\tt TP1/Poisson\_OpenMP\_fine\_grain}, paralléliser avec OpenMP grain fin:
