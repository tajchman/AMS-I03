...
\begin{frame}
	...
	Règle: \textcolor{red}{les zones mémoire modifiées par deux threads différents au même instant doivent être distinctes (sinon les résultats peuvent être faux) et assez éloignées (pour diminuer le cout du maintien de la cohérence de cache)}
	
	\vfill
	En théorie, il faut une distance d'au moins la taille d'une ligne de cache dans le cache L1.
	En pratique, on  
\end{frame}
