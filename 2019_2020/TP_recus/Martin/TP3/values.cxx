...
void Values::init(double (*f)(double, double, double))
{
  ...
  if (f) {
    ...
    //on voudrait paralleliser le boucle plus petit pour optimiser le temps. 
	//Pour ça il faudrait comparer la taille des boucles. 
	//Le code plus performante serait celui capable de ordoner les boucles selon la taille
	//de plus petit à plus grand, et paralleliser le premier
	//Ici on suppose que le plus petit est le boucle i, apres le boucle j et
	//le plus grand est le boucle sur k
	//On fera la même suposition pour tous les boucles for notés avec une étoile

	//*
	#pragma omp parallel for private(i,j,k) shared(dx,dy,dz,xmin,ymin,zmin)
    for (i=imin; i<imax; i++)
      for (j=jmin; j<jmax; j++)
        for (k=kmin; k<kmax; k++)
