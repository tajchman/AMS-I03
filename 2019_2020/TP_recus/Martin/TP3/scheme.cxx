...
double Scheme::iteration()
{
  ...
  du_sum = 0.0;
#pragma omp parallel for private(i,j,k,du) shared(du_sum,di,dj,dk,m_lambda) reduction(+:du_sum)
  for (i = imin; i < imax; i++)
    for (j = jmin; j < jmax; j++)
      for (k = kmin; k < kmax; k++) {
...
bool Scheme::solve(unsigned int nSteps)
{
  ...
  int iStep;
#pragma omp parallel for private(m_duv,i) shared(m_t,kStep,m_dt,m_v) reduction(+:m_t,kStep)
