...
void copy(std::vector<Network::TrainingDataGPU> & t1,
               const std::vector<Network::TrainingDataCPU> & t2)
{
}

void Network::SGD(std::vector<TrainingDataCPU> &training_data_cpu,
                  int epochs, int mini_batch_size, double eta,
                  std::vector<TrainingDataCPU> &test_data_cpu)
{
  std::vector<TrainingDataGPU> training_data;
  copy(training_data, training_data_cpu);
  std::vector<TrainingDataGPU> test_data;
...
void Network::update_mini_batch(TrainingDataGPUIterator td,
                       int mini_batch_size, double eta) {
  BiasesVector nabla_b(b0);
//   std::vector<Matrix> nabla_w(w0);
//   for (auto i = 0; i < mini_batch_size; ++i) {
//     TrainingDataGPUIterator tdi = td + i;
//     auto &x = tdi->first;  // test data
//     auto &y = tdi->second; // expected result
//     std::vector<Vector> delta_nabla_b(b0);
//     std::vector<Vector> delta_nabla_w(w0);
...
int Network::evaluate(const std::vector<TrainingDataGPU> &td) const {
  // return count_if
  //   (td.begin(), td.end(),
  //    [this](const TrainingDataGPU &testElement) {
