...
\section{Code OpenMP `grain grossier`}

Le fichier {\tt src/sinus\_openmp\_fine/sinus.cxx} calcule une approximation par série de Taylor de la fonction $x \mapsto \sin(x)$ pour un ensemble de valeur de $x \in [0, 2\pi]$. Les résultats sont sauvegardés dans un fichier.
...
\begin{question}
	...
	\begin{itemize}
		...
		\begin{script}
			./build/sinus\_seq/sinus\_seq 10000000
		\end{script}
		\item la version multi-threads en utilisant 1 thread
		\begin{script}
	OMP\_NUM\_THREADS=1 \textbackslash
	\quad ./build/sinus\_openmp\_1/sinus\_openmp\_1 10000000
		\end{script}
...
\item Recompiler le code. Il suffit de se placer dans {\tt code/PoissonOpenMP} et de taper :
\begin{script}
        make -C build
\end{script}
...
\item Comparer les résultats et temps calcul affichés avec ceux de la version séquentielle. Si les résultats sont différents, c'est qu'il y a une erreur dans les intractions de parallélisation. Corriger et reprendre en 3.
\end{enumerate}
\end{question}
