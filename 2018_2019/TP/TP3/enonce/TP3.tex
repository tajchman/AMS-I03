...
\begin{question}
    Ajouter aux pragmas OpenMP ``for'' de la version ``grain fin'', l'option {\tt schedule(static, n/12)}.
    Comparer les temps d'exécution avec les versions précédentes en tapant la commandes

    Faire varier le paramètre {\tt n/12}.

    Remplacer dans l'option schedule, le paramètre {\tt  static} par {\tt dynamic}. Refaire les comparaisons en temps d'exécution.
\end{question}
