...
\begin{itemize}
	\item une version ``release'', optimisée par le compilateur (répertoire \newline{\tt install/Release}),
	\item une version ``debug'' permettant le suivi de l'exécution par un ``debugger'' et qui donne plus d'informations avec les outils présentés plus loin (répertoire {\tt install/Debug}),
...
\begin{itemize}
	...
	\item Pour (essayer d') améliorer le temps d'exécution : utiliser la version profile ou la version debug + outil de mesure du temps
	\item Quand on est satisfait de l'état du code : utiliser la version optimisée.
\end{itemize}
\end{remarque}
    ...
    \begin{itemize}[label=\textbullet, parsep=1pt]
        ...
        \item Fonctions définies par le langage et utilisables depuis l'intérieur du code : 
        ...
        \item Outils externes de ``profilage'', ajoutent automatiquement des points de mesure dans le code (gprof), s'interposent entre le code et le système pour récuperer des informations (valgrind, perf)
    ...
    Cette classe est utilisée dans le fichier source {\tt src/valeur\_propre/power1.cxx} qui initialise une matrice $100\times100$ et calcule sa plus grande valeur propre (en valeur absolue), par la méthode de la puissance : 
    ...
    \begin{question}    	
    	...
    	Exécutez le code {\tt install/Release/power1} (compilé au début du TP). Le code affiche, entre autres, le temps d'initialisation de la matrice, le temps de calcul de la valeur propre et le temps total de calcul.
...
\begin{question}
	...
	Expliquer les différences de temps calcul en examinant les fichiers sources C++ {\tt src/valeur\_propre/power1.cxx} (utilisé dans le code power1) \\ et {\tt src/valeur\_propre/power2.cxx} (utilisé dans le code power2).
\end{question}
...
\begin{question}
	Faire la même comparaison avec les codes {\tt install/Release/power1f} et \linebreak {\tt install/Release/power2f}, qui utilisent respectivement des sources fortran {\tt src/valeur\_propre/power1.f90} et {\tt src/valeur\_propre/power2.f90}.
        ...
        Expliquer les différences de résultats entre la version C++ ({\tt power1.cxx} et {\tt power2.cxx}) et fortran 90 ({\tt power1.f90} et {\tt power2.f90}).
\end{question}
...
Expliquer les différences éventuelles de temps calcul en examinant les fichiers sources C++ {\tt src/valeur\_propre/power2.cxx} (utilisé dans le code power2) \\ et {\tt src/valeur\_propre/power3.cxx} (utilisé dans le code power3).
\end{question}
...
\begin{question}
	...
	Expliquer les différences éventuelles de temps calcul en examinant les fichiers sources C++ {\tt src/valeur\_propre/power3.cxx} (utilisé dans le code power3) \\ et {\tt src/valeur\_propre/power4.cxx} (utilisé dans le code power4).
\end{question}
...
\begin{question}
	...
	Comparer les temps de calcul et expliquer les différences en examinant les fichiers source \\ {\tt src/transposee/transpose1.cxx} et {\tt src/transposee/transpose2.cxx}.
\end{question}
...
\begin{question}
	...
	Comparer les temps de calcul avec les 2 versions précédentes et expliquer les différences en examinant le fichier source {\tt src/transposee/transpose3.cxx}.
\end{question}
...
\begin{question}
	...
	Comparer les temps de calcul avec la version précédente et expliquer les différences en examinant le fichier source {\tt src/transposee/transpose4.cxx}.
	\end{question}
\end{document}
